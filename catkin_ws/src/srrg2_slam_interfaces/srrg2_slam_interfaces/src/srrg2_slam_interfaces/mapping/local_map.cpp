...
namespace srrg2_slam_interfaces {
  ...
  void DynamicPropertyContainerOwner::_drawImpl(ViewerCanvasPtr canvas_) const {
    if (!canvas_) {
      return;
    }
  ...
  void LocalMap_<VariableType_>::_drawImpl(srrg2_core::ViewerCanvasPtr canvas) const {
    if (!canvas) {
      return;
    }
    canvas->pushMatrix();
