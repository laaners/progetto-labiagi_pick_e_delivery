...
namespace srrg2_slam_interfaces {
  ...
  /**
   * @brief Visibility-based splitting criterion
  ...
  public:
    ...
    using TrackerTypePtr     = std::shared_ptr<typename SlamAlgorithmType_::TrackerType>;
    using RelocalizerTypePtr = std::shared_ptr<typename SlamAlgorithmType_::RelocalizerType>;
    ...
    virtual void compute() override {
      ...
      TrackerTypePtr tracker = ThisType::_slam->param_tracker.value();
      if (!tracker) {
    ...
    void reset() override {
      LocalMapSplittingCriterionBase_<SlamAlgorithmType_>::reset();
      _previous_number_of_points=0;
    }
  protected:
