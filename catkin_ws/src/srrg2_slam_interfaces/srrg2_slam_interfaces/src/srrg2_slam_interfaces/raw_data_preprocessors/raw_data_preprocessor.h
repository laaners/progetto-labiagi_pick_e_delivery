...
namespace srrg2_slam_interfaces {
  ...
  public:
    ...
    virtual bool setRawData(srrg2_core::BaseSensorMessagePtr raw_data_,
                            TrackerReportRecord& report_record_) {
      _raw_data_changed_flag = true;
    ...
    bool fillReport(TrackerReportRecord& report_record_, srrg2_core::BaseSensorMessagePtr measurement){
      TrackerInputHandlePtr handle(new TrackerInputHandle(measurement->topic.value(),
                                                          measurement->seq.value(),
                                                          measurement->timestamp.value()));
      report_record_.measurements.push_back(handle);
      return true;
    }
