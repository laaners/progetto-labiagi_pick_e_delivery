...
namespace srrg2_slam_interfaces {
  ...
  void MultiTrackerBase_<EstimateType_>::preprocessRawData() {
     if (_slice_processors_changed_flag) {
      setScene(_scene);
      _slice_processors_changed_flag=false;
      if (_scene==&_dummy_scene) {
        std::cerr << FG_RED("TrackerStandard_::_preprocessRawData, running in standalone mode") << std::endl;
        populateScene(_dummy_scene);
      }
    }
  ...
  void MultiTrackerBase_<EstimateType_>::setRawData(BaseSensorMessagePtr message_) {
    ...
    for (size_t i = 0; i < param_slice_processors.size(); ++i) {
      ...
      proc->setRawData(message_, *(this->_report_record));
    }
  }
  ...
  template <typename EstimateType_>
  void MultiTrackerBase_<EstimateType_>::_drawImpl(srrg2_core::ViewerCanvasPtr canvas_) const {
  ...
  template <typename EstimateType_>
  void MultiTrackerBase_<EstimateType_>::reset() {
    BaseType::reset();
    PROFILE_TIME("MultiTrackerBase::reset");
    //    const EstimateType_ tracker_from_scene(_robot_in_local_map.inverse());
    for (size_t index_slice = 0; index_slice < param_slice_processors.size(); ++index_slice) {
      SliceProcessorTypePtr slice = param_slice_processors.value(index_slice);
      if (slice)
