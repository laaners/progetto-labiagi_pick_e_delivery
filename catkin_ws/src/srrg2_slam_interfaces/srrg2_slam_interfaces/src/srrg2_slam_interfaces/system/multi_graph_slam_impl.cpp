...
namespace srrg2_slam_interfaces {
  ...
  void MultiGraphSLAM_<LoopClosureType_>::makeNewMap(float info_scale_) {
    ...
    if (previous_local_map) {
      factor = FactorBaseTypePtr(new FactorBaseType);
    ...
    _current_local_map_msg =
      LocalMapMessageTypePtr(new LocalMapMessageType(*_current_local_map,
                                                     LocalMapMessageType::LocalMapActions::CREATE,
                                                     "/local_map_update",
                                                     "multi_graph_slam",
                                                     _message->seq.value(),
                                                     _message->timestamp.value()));
    _slam_update_messages.push_back(_current_local_map_msg);
    if (!factor) {
      return;
    }

    FactorMessageTypePtr factor_message(new FactorMessageType(*factor,
                                                              "/local_map_update",
                                                              "multi_graph_slam",
                                                              _message->seq.value(),
                                                              _message->timestamp.value()));
    _slam_update_messages.push_back(factor_message);
  }
  ...
  void MultiGraphSLAM_<LoopClosureType_>::compute() {
    ...
    switch (tracker->status()) {
      ...
      case TrackerBase::Lost:
        ...
        std::flush(std::cerr);
        makeNewMap(0.1);
        // srrg this could be the prior instead TODO allow the tracker to fallback to prior only
        // estimation in case of lost
    ...
    tracker->setLocalMapInWorld(_current_local_map->estimate());
    tracker->merge();
    tracker->updateReport();
    if (!_current_local_map_msg) {
      _current_local_map_msg =
        LocalMapMessageTypePtr(new LocalMapMessageType(*_current_local_map,
                                                       LocalMapMessageType::LocalMapActions::TRACK,
                                                       "/local_map_update",
                                                       "multi_graph_slam",
                                                       _message->seq.value(),
                                                       _message->timestamp.value()));
    ...
    _current_local_map_msg->history.push_back(tracker->report());
    ++_number_of_processed_measurements;
      ...
      switch (stats->status) {
        ...
        case FactorGraphClosureValidator::ClosureStats::Status::Accepted:
          ...
          if (std::binary_search(detected_closures.begin(), detected_closures.end(), closure)) {
            ...
            FactorMessageTypePtr factor_message(new FactorMessageType(*closure,
                                                                      "/local_map_update",
                                                                      "multi_graph_slam",
                                                                      _message->seq.value(),
                                                                      _message->timestamp.value()));
  ...
  void MultiGraphSLAM_<LoopClosureType_>::optimize() {
    ...
    NodeUpdateMessageTypePtr node_update_message(new NodeUpdateMessageType(
      "/node_update", "multi_graph_slam", _message->seq.value(), _message->timestamp.value()));
    for (auto it : _graph->variables()) {
      const srrg2_solver::VariableBase* v = it.second;
      const LocalMapType* lmap            = dynamic_cast<const LocalMapType*>(v);
      if (!lmap) {
        continue;
      }
      node_update_message->updates.insert(std::make_pair(lmap->graphId(), lmap->estimate()));
    }
    _slam_update_messages.push_back(node_update_message);
  }
  ...
  bool MultiGraphSLAM_<LoopClosureType_>::putMessage(BaseSensorMessagePtr message_) {
    setRawData(message_);
    compute();
    bool computed=!_slam_update_messages.empty();
    while (!_slam_update_messages.empty()) {
      BaseSensorMessagePtr msg = _slam_update_messages.front();
      _slam_update_messages.pop_front();
      if (param_push_sinks.size()) {
        computed |=propagateMessage(msg);
      }
    }
    if (computed) {
      this->_need_redraw=true;
  ...
  template <typename LoopClosureType_>
  void MultiGraphSLAM_<LoopClosureType_>::_drawImpl(ViewerCanvasPtr canvas) const {
  ...
  template <typename LoopClosureType_>
  void MultiGraphSLAM_<LoopClosureType_>::reset() {
    _slam_update_messages.clear();
    _robot_in_local_map.setIdentity();
    _current_local_map = nullptr;
    if (_graph) {
      _graph->clear();
    }
    _relocalized_closure.reset();
    _number_of_processed_measurements = 0;
    _message.reset();
    if (param_global_solver.value()) {
      param_global_solver->reset();
    }
    if (param_tracker.value()) {
      param_tracker->reset();
    }
    if (param_loop_detector.value()) {
      param_loop_detector->reset();
    }
    if (param_closure_validator.value()) {
      param_closure_validator->reset();
    }
    if (param_splitting_criterion.value()) {
      param_splitting_criterion->reset();
    }
    if (param_relocalizer.value()) {
      param_relocalizer->reset();
    }
    if (param_initializer.value()) {
      param_initializer->reset();
    }
