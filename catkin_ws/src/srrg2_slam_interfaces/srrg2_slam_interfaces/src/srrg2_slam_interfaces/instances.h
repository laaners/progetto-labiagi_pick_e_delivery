...
#include "system/map_listener.h"
#include "trackers/multi_tracker.h"
#include "trackers/tracker_report.h"
...
namespace srrg2_slam_interfaces {
  ...
  // messages
  using LocalMapMessage2D      = LocalMapMessage_<MultiGraphSLAM2D::LocalMapType>;
  using LocalMapMessage2DPtr   = std::shared_ptr<LocalMapMessage2D>;
  using NodeUpdateMessage2D    = NodeUpdateMessage_<MultiGraphSLAM2D::LocalMapType>;
  using NodeUpdateMessage2DPtr = std::shared_ptr<NodeUpdateMessage2D>;
  using FactorMessageSE2       = FactorMessage_<MultiGraphSLAM2D::FactorBaseType>;
  using FactorMessageSE2Ptr    = std::shared_ptr<FactorMessageSE2>;

  using LocalMapMessage3D      = LocalMapMessage_<MultiGraphSLAM3D::LocalMapType>;
  using LocalMapMessage3DPtr   = std::shared_ptr<LocalMapMessage3D>;
  using NodeUpdateMessage3D    = NodeUpdateMessage_<MultiGraphSLAM3D::LocalMapType>;
  using NodeUpdateMessage3DPtr = std::shared_ptr<NodeUpdateMessage3D>;
  using FactorMessageSE3       = FactorMessage_<MultiGraphSLAM3D::FactorBaseType>;
  using FactorMessageSE3Ptr    = std::shared_ptr<FactorMessageSE3>;
