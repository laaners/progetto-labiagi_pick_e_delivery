...
#include <srrg_solver/solver_core/solver_action_draw.h>
...
void doOptimization(FactorGraphPtr factor_graph_,
  ...
  SolverActionDrawPtr draw_action(new SolverActionDraw);
  draw_action->addCanvas(canvas_);
  draw_action->param_event.setValue(srrg2_solver::Solver::IterationEnd);
  sparse_solver_->param_actions.pushBack(draw_action);
  ...
  while (srrg2_qgl_viewport::ViewerCoreSharedQGL::isRunning()) {
    ...
    if (sparse_solver_->status() == Solver::SolverStatus::Error) {
      srrg2_qgl_viewport::ViewerCoreSharedQGL::stop();
      break;
    }

    if (sparse_solver_->status() == Solver::SolverStatus::Success) {
      continue;
    }
