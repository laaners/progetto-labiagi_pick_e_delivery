...
namespace srrg2_solver {
  ...
  void getTransform(Isometry3f& dest, const Isometry3f& src) {
    dest=src;
  }
  
  void getTransform(Isometry2f& dest, const Isometry3f& src) {
    dest.linear()=src.linear().block<2,2>(0,0);
    dest.translation()=src.translation().head<2>();
  }

  void getTransform(Similiarity3f& dest, const Isometry3f& src) {
    dest.setIdentity();
