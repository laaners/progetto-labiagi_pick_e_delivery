...
namespace srrg2_solver {
  ...
  void Solver::computeActiveFactors(bool inlier_only) {
    ...
    for (auto& l_it: _active_factors) {
      ...
      std::sort(l_it.second.begin(),
                l_it.second.end(),
                [](const FactorBase* a, const FactorBase* b) -> bool {return a->graphId()<b->graphId();});
  ...
  bool Solver::lambdaRatio(float& lambda_min,
                           float& lambda_max,
                           VariableBase* v, int level_) {
    ...
    if (Hxx->rows()!=v->perturbationDim()) {
      throw std::runtime_error("dim mismatch" + to_string(dim) + " instead of" + to_string(Hxx->rows()));
    }
    Hxx->setZero();
    for (auto f_it:_graph->factors(v)) {
      ...
      bool active=std::binary_search(_active_factors[_current_level].begin(),
                                     ...
                                     f, [](const FactorBase* a, const FactorBase* b)->bool
  ...
  void Solver::compute() {
    ...
    if (_compute_changed_flag) {
      ...
      if (!_max_iterations_total) {
        std::cerr << FG_YELLOW("Solver::compute|number of iterations == 0")
                  << std::endl;
      }
    ...
    for (_current_level = highest_level; _current_level >= 0; --_current_level) {
      ...
      for (auto it: _graph->factors()) {it.second->_current_level=_current_level;}
      assignRobustifiers();
  ...
  void Solver::initActions() {
    ...
    std::sort(actions.begin(), actions.end(), [] (const SolverActionBasePtr& a1,
                                                  const SolverActionBasePtr& a2) ->bool {
                if (! a1 && !a2)
                  return false;
                if (a1 &&! a2)
                  return true;
