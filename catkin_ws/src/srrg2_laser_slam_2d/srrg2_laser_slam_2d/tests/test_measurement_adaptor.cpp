...
TEST_F(Synthetic, MeasurementAdaptor) {
  ...
  adaptor.param_unprojector->setCanvasCols(number_of_measurements);
  adaptor.param_unprojector->setCanvasRows(1);
  ...
  adaptor.setRawData(std::dynamic_pointer_cast<BaseSensorMessage>(laser_message_circle), report_record);
  adaptor.compute();
...
TEST_F(AIS, MeasurementAdaptorKillianCourt) {
  ...
  adaptor.param_unprojector->setCanvasCols(laser_message_0->ranges.size());
  adaptor.param_unprojector->setCanvasRows(1);
  ...
  adaptor.setMeas(&points);
  srrg2_slam_interfaces::TrackerReportRecord report_record;
  adaptor.setRawData(benchamin->getMessage(), report_record);
  adaptor.compute();
