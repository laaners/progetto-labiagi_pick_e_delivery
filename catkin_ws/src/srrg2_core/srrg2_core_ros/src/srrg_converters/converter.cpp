...
namespace srrg2_core_ros {
  ...
  srrg2_core::BaseSensorMessagePtr Converter::convert(turtlesim::PoseConstPtr message_in_) {
    if (!message_in_) {
      return nullptr;
    }
    ...
    v << message_in_->x, message_in_->y, message_in_->theta;
    srrg2_core::Isometry3f pose =
      srrg2_core::geometry3d::get3dFrom2dPose(srrg2_core::geometry2d::v2t(v));
    message_out->pose.setValue(pose);
    message_out->linear_velocity.setValue(srrg2_core::Vector3f(message_in_->linear_velocity, 0, 0));
    message_out->angular_velocity.setValue(
      srrg2_core::Vector3f(0, 0, message_in_->angular_velocity));
  ...
  srrg2_core::BaseSensorMessagePtr Converter::convert(grid_map_msgs::GridMapConstPtr message_in_) {
    ...
    message_out->timestamp.setValue(message_in_->info.header.stamp.toNSec());
    ...
    for (size_t i = 0; i < message_in_->layers.size(); ++i) {
      ...
      if (message_in_->data[i].layout.dim.size() == 2) {
        ...
        //        if (current_layer_name == "cost_map") {
        //          std::ofstream myfile;
        //          myfile.open("my_matrix_after_ros_srrg_conversion.txt");
        //          for (size_t row = 0; row < property_layer->value().rows(); ++row) {
        //            for (size_t col = 0; col < property_layer->value().cols(); ++col) {
        //              myfile << property_layer->value().at(row, col) << std::endl;
  ...
  srrg2_core::BaseSensorMessagePtr Converter::convert(srrg2_core_ros::PlannerStatusMessageConstPtr message_in_) {
  ...
  srrg2_core_ros::PlannerStatusMessagePtr Converter::convert(srrg2_core::PlannerStatusMessagePtr message_in_){
