...
class RosNode extends EventEmitter {
  ...
  */
    ...
    return new Promise((resolve, reject) => {
      ...
      this.lookupService(serviceName).then(resp => {
        ...
        const client = net.connect(serviceHost, () => {
          const serviceClientHeader = tcprosUtils.createServiceClientHeader(this._nodeName, serviceName, '*', '*', false);
  ...
  _setupUdprosServer() {
    let udprosPort = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('message', (dgramMsg, rinfo) => {
        ...
        // first dgram message
        const connectionId = header.connectionId;
  ...
  _handleGetBusInfo(err, params, callback) {
    ...
    Object.keys(this._subscribers).forEach(topic => {
      ...
      sub.getClientUris().forEach(clientUri => {
        busInfo.push([++count, clientUri, 'i', sub.getTransport(), sub.getTopic(), true]);
      });
    });
    ...
    Object.keys(this._publishers).forEach(topic => {
      ...
      pub.getClientUris().forEach(clientUri => {
        busInfo.push([++count, clientUri, 'o', pub.isUdpSubscriber(clientUri) ? 'UDPROS' : 'TCPROS', pub.getTopic(), true]);
      });
    });
  ...
  _setupExitHandler(forceExit) {
    ...
    let exitImpl = function exitImpl() {
      ...
      // while unregistering
      const promises = [shutdownServer(this._slaveApiServer, 'slaveapi'), shutdownServer(this._tcprosServer, 'tcpros'), shutdownServer(this._udprosServer, 'udpros')];
