...
DeserializeStream.prototype = {
  _transform: function _transform(chunk, encoding, done) {
    var _this = this;
    ...
    while (pos < chunkLen) {
      if (this._inBody) {
        ...
        (function () {
          ...
          if (_this._deserializeServiceResp && _this._serviceRespSuccess === null) {
            _this._serviceRespSuccess = chunk.readUInt8(pos, true);
            ++pos;
          }

          var bufLen = 0;
          _this._messageBuffer.forEach(function (bufferEntry) {
            bufLen += bufferEntry.length;
          });

          // first 4 bytes of the message are a uint32 length field
          if (chunkLen - pos >= 4 - bufLen) {
            _this._messageBuffer.push(chunk.slice(pos, pos + 4 - bufLen));
            var buffer = Buffer.concat(_this._messageBuffer, 4);
            _this._messageLen = buffer.readUInt32LE(0);
            pos += 4 - bufLen;
            ...
            if (_this._messageLen === 0 && pos === chunkLen) {
              _this.emitMessage(new Buffer([]));
            } else {
              _this._inBody = true;
            }
          } else {
            // the length field is split on a chunk
            _this._messageBuffer.push(chunk.slice(pos));
            pos = chunkLen;
          }
        })();
      }
  ...
  emitMessage: function emitMessage(buffer) {
  ...
  setServiceRespDeserialize: function setServiceRespDeserialize() {
...
// HERE: this overwrites _transform with a "not implemented"
